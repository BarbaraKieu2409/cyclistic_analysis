...
/*
 * Based on the result in above, I will state that almose casual use bycycle between 8 and 27 minutes, 
 * and almost member use bycycle between 5 and 16 minutes in quarter 2*/
...
-- Stats casual which ride_length > 27 minutes and < 8 minutes | 8 <= ride_length <= 27 minutes by rideable_type
SELECT 
	'quarter_2' AS quarter_name,
	member_casual,
	rideable_type,
	'use more than 27m' AS ride_length,
	COUNT(*) AS total
FROM quarter_2
WHERE 
	TIME_TO_SEC(ride_length) > 27*60
	AND
...
SELECT 
	...
	member_casual,
	rideable_type,
	'use less than 8m' AS ride_length,
	COUNT(*) AS total
FROM quarter_2
WHERE 
	TIME_TO_SEC(ride_length) < 8 * 60
	AND
	member_casual = 'casual'
GROUP BY rideable_type
UNION 
SELECT
	...
	member_casual,
	rideable_type,
	'use between 8 and 27m' AS ride_length,
	COUNT(*) AS total
FROM quarter_2
WHERE 
	TIME_TO_SEC(ride_length) >= 8 * 60
